...
export const COLOR_PRESETS: ColorPreset[] = [
  { id: 'green',  name: 'Verde marca', swatch: '#0E6B4D' },
...
const PALETTES: Record<string, ColorPatch> = {
  green: {
    vars: {
      ...
      '--grad-card':    'linear-gradient(125deg, #15803d 0%, #22c55e 45%, #B85C3A 130%)',
      '--grad-accent':  'linear-gradient(90deg, #22c55e, #B85C3A)',
    },
  ...
  purple: {
    vars: {
      ...
      '--grad-card':    'linear-gradient(125deg, #6d28d9 0%, #7c3aed 45%, #B85C3A 130%)',
      '--grad-accent':  'linear-gradient(90deg, #7c3aed, #B85C3A)',
    },
  ...
  pink: {
    vars: {
      ...
      '--grad-card':    'linear-gradient(125deg, #be185d 0%, #ec4899 45%, #B85C3A 130%)',
      '--grad-accent':  'linear-gradient(90deg, #ec4899, #B85C3A)',
    },
    darkVars: { '--blue-50': '#500724', '--blue-300': '#f472b6' },
  },
  orange: {
    vars: {
      '--blue-700': '#c2410c', '--blue-600': '#ea580c', '--blue-500': '#B85C3A',
      '--blue-300': '#fdba74', '--blue-100': '#ffedd5', '--blue-50':  '#fff7ed',
      '--shadow-blue':  '0 8px 20px rgba(194,65,12,0.28)',
      '--grad-brand':   'linear-gradient(150deg, #c2410c, #B85C3A 60%, #9a3412)',
      '--grad-card':    'linear-gradient(125deg, #c2410c 0%, #B85C3A 45%, #eab308 130%)',
      '--grad-accent':  'linear-gradient(90deg, #B85C3A, #eab308)',
    },
...
function buildCustomCSS(hex: string): string {
  ...
  return `:root{--blue-700:${hex};--blue-600:${hex};--blue-500:${hex};--blue-300:${light300};--blue-50:${light50};--shadow-blue:${shadow};--grad-brand:linear-gradient(150deg,${hex},${hex} 60%,${hex});--grad-card:linear-gradient(125deg,${hex} 0%,${hex} 45%,#B85C3A 130%);--grad-accent:linear-gradient(90deg,${hex},#B85C3A);}`;
}
...
export function getUserColorScheme(userId: string): string {
  return localStorage.getItem(`fm_color_scheme_${userId}`) || 'green';
}
